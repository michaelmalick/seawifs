...
#####################################################################
# This script computes octads (8-day) from daily resolution chl-a data
# recorded by SeaWiFS. 
#
# To run this script:
#   - Change working directory to this folder
#   - chmod a+x 8day_write.csh
#   - ./8day_write.csh
#
#
# Michael Malick
...
set THIS_DIR = `pwd`
set OUT_DIR = $THIS_DIR/8day_data
...
while ( $yr <= 2010 )
    ...
    while ( $pen_ind <= 46 ) # throw-out december 31 of each yr
		
		# account for leap years
		if ( $leap == 0 && $pen_ind == 7 ) then
		    set ind2 = 9
		else
		    set ind2 = 8
		endif

        # Last octad only has 5 days
        if ( $pen_ind == 46 ) then
            set ind2 = 5
        endif
	...
	# concatenate all temp files for a year
