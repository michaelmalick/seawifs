...
#####################################################################
# This script computes 7-day means from daily resolution chl-a data
# recorded by SeaWiFS. The input data for this script is the
# subsetted oregon-coast 1day data located in the 1Day_data directory 
#
# To run this script:
#   - Change working directory to this folder
#   - chmod a+x 7day_write.csh
#   - ./7day_write.csh
#
#
# Michael Malick
...
set THIS_DIR = `pwd`
set OUT_DIR = /Users/malickmj/Desktop/seawifs/oregon_subset/7day_data 
set SRC_DIR = /Users/malickmj/Desktop/seawifs/oregon_subset/1day_data
...
while ( $yr <= 2010 )
    ...
    while ( $pen_ind <= 52 ) # throw-out december 31 of each yr
		
		# account for leap years
		if ( $leap == 0 && $pen_ind == 8 ) then
		    set ind2 = 8
		else
		    set ind2 = 7
		endif
		...
		@ ind2a = $day_ind + $ind2 - 1 # high (7)

		# create average and save to temp file
		ncra -H -h $NCOOPTS -d time,$ind1a,$ind2a $fname $temfil

	    @ day_ind = $day_ind + $ind2 # move day counter ahead 7
		@ pen_ind = $pen_ind + 1 # move 7day counter ahead 1

    end

	# concatenate all temp pentad files for a year
	echo "Concatenating files for $yr"
    ncrcat -O -H -h $SRC_DIR/temfil.??.nc $OUT_DIR/7day_chloro_$yr.nc
