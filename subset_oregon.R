...
# seawifs data and subsets the area to the Oregon coast. It then 
...
#   /Users/malickmj/Documents/data/seawifs
#
# Seawifs data files are in HDF4 format
# Level-3 mapped data products for 1998-2010
# Map subset 40N to 50N and -135W to -120W (Oregon Coast)
#
...
for(i in 1999:2010) {
    ...
    for(j in days) {
        ...
        # Subset lat and lon 
        lat.min <- which(x$lat > 39.99 & x$lat < 40.01)
        lat.max <- which(x$lat > 49.99 & x$lat < 50.01)
        lon.min <- which(x$lon < -134.99 & x$lon > -135.01)
        ...
        chl.nc <- create.ncdf(paste(f.name, "sub.nc", sep = ""), chl)
        ...
        att.put.ncdf(chl.nc, varid = 0, attname = "history", 
            ...
            chlorophyll-a in mg m-3 for the Oregon Coast 
            (40-50N, 135-120W).  The data are the
    ...
    system(paste("ncrcat -h ", "S", i, "???sub.nc ", "S", i, "sub.nc",
        sep = ""))

    # Delete daily subsetted .nc files
    system(paste("rm ", "S", i, "???sub.nc ", sep = ""))
